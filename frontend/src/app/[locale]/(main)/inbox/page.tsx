...
function InboxContent() {
  ...
  const handleCancelSchedule = async (reason: string) => {
    if (!cancellingRequestId || !reason.trim()) return;
    ...
    try {
      ...
      showToast(t("chat.cancel_success_toast") || "Successfully cancelled");
      setCancelDialogOpen(false);
    } catch (err: any) {
      showToast(err.message || "Failed to cancel schedule");
    } finally {
  ...
  const listPane = (
    ...
    >
      <div className="p-4 border-b border-border">
        <h1 className="text-2xl font-bold text-foreground mb-3">{t("chat.title")}</h1>
        ...
        {loadingList ? (
          ...
          <div className="p-8 text-center">
            ...
            <p className="font-semibold text-foreground mb-2">
              {search ? t("chat.no_match") : t("chat.empty_conversations")}
            </p>
            {!search && (
              <>
  ...
  const chatPane = !selected ? (
    <div className="hidden md:flex flex-1 items-center justify-center text-center p-8">
      <div>
        ...
        <p className="text-lg font-semibold text-foreground">{t("chat.select_chat")}</p>
        <p className="text-sm text-muted mt-1">{t("chat.select_chat_hint")}</p>
      </div>
    </div>
  ) : (
    <div className={cn("flex flex-col h-full flex-1 min-w-0", !selectedId && "hidden md:flex")}>
      ...
      <div className="flex items-center gap-3 border-b border-border px-4 py-3 bg-surface">
            ...
            <p className="text-xs text-muted flex items-center gap-1.5 flex-wrap">
              ...
              {partnerTz && (
                <span className="hidden sm:inline-flex items-center gap-1">
                  ...
                  {myTz.flag} {currentTimeAt(myTz.offset)} {t("chat.user_local_time")}
                </span>
              )}
            </p>
          </div>
        ...
        <div className="flex items-center gap-1">
          ...
          <button
            ...
            className="p-2 text-muted hover:text-primary rounded-full hover:bg-primary/10 transition-colors"
            title={t("chat.trans_modal_tooltip")}
          >
            ...
            {menuOpen && (
              <>
                ...
                <div className="absolute right-0 top-11 z-20 w-48 rounded-2xl border border-border bg-surface shadow-xl py-2 animate-in fade-in zoom-in-95 duration-150">
                  ...
                  >
                    <UserRound className="h-4 w-4 text-muted" /> {t("chat.view_profile")}
                  </Link>
                  <button
                  ...
                  >
                    <Flag className="h-4 w-4 text-warning" /> {t("chat.report")}
                  </button>
                  <button
      ...
      >
        {selectionSave && (
          ...
          >
            <BookOpen className="w-3.5 h-3.5" /> {t("chat.save_word_popup")}
          </button>
        )}
        {loadingMessages ? (
          ...
          <div className="text-center py-12">
            ...
            <p className="font-semibold text-foreground">{t("chat.matched_title")}</p>
            <p className="text-sm text-muted mt-1">{t("chat.matched_desc")}</p>
          </div>
        ) : (
          messages.map((m, i) => {
                  ...
                  >
                    <div className="flex items-center gap-2 mb-2">
                      ...
                      <span className="text-xs font-bold text-foreground">
                        {mine
                          ? t("chat.invite_sent")
                          : t("chat.invite_received", { name: selected.partner.displayName })}
                      </span>
                    </div>
                    {isNewShape ? (
                      <>
                        <p className="text-sm font-semibold text-foreground">🕐 {localTime}</p>
                        <p className="text-[11px] text-muted mt-0.5">{t("chat.local_time_hint")}</p>
                      </>
                    ) : (
                      <>
                    ...
                    {!mine && sd.status === "pending" && (
                      <div className="flex gap-2 mt-3">
                        ...
                        >
                          <Check className="w-3 h-3 inline mr-1" /> {t("chat.accept")}
                        </button>
                        <button
                          onClick={() =>
                            isNewShape
                        ...
                        >
                          <X className="w-3 h-3 inline mr-1" /> {t("chat.decline")}
                        </button>
                      </div>
                    )}
                    {isNewShape && (sd.status === "accepted" || (sd.status === "pending" && mine)) && (
                      <button
                        onClick={() => openCancelDialog(sd.requestId!)}
                        className="mt-3 w-full rounded-full py-1.5 px-3 text-xs font-semibold border border-error/30 text-error hover:bg-error/5 transition-all text-center flex items-center justify-center gap-1"
                      >
                        <X className="w-3 h-3" /> {t("chat.cancel")}
                      </button>
                    )}
                    ...
                    {sd.status !== "pending" && (
                      <div className="mt-2">
                        ...
                        >
                          {sd.status === "accepted"
                            ? t("chat.status_accepted")
                            : sd.status === "expired"
                              ? t("chat.status_expired")
                              : sd.status === "cancelled"
                                ? t("chat.status_cancelled")
                                : t("chat.status_declined")}
                        </p>
                        {sd.status === "cancelled" && sd.cancelReason && (
                          <p className="text-[11px] text-muted-foreground mt-1 bg-muted/40 p-2 rounded border border-border">
                            {t("chat.cancel_reason_label", { reason: sd.cancelReason })}
                          </p>
                        )}
                      </div>
                    )}
              ...
              <div key={m.id} className={cn("flex group", mine ? "justify-end" : "justify-start")}>
                <div className="max-w-[75%]">
                  ...
                  >
                    ...
                    {isLastMine && (
                      <span>· {m.pending ? t("chat.status_sending") : m.readAt ? t("chat.status_read") : t("chat.status_sent")}</span>
                    )}
                  </div>
                      ...
                      {!mine && m.type === "text" && (
                        <>
                          ...
                          >
                            ...
                            {translating[m.id] ? t("chat.translating") : showTranslationFor[m.id] ? t("chat.hide") : t("chat.translate")}
                          </button>
                          <button
                          ...
                          >
                            <BookOpen className="w-3 h-3" /> {t("chat.save_word")}
                          </button>
                          <button
                            onClick={() => {
                            ...
                            className="flex items-center gap-1 text-[10px] font-semibold rounded-full px-2.5 py-1 bg-surface border border-border text-muted hover:border-primary hover:text-primary transition-all"
                          >
                            <Languages className="w-3 h-3" /> {t("chat.trans_modal_tooltip")}
                          </button>
                        </>
                      )}
                    </div>
      ...
      {selected && (
        <>
          <ReportDialog
            ...
            onDone={() => showToast(t("chat.report_success"))}
          />
          <BlockDialog
            open={blockOpen}
            ...
            targetName={selected.partner.displayName}
            onDone={() => {
              showToast(t("chat.block_success", { name: selected.partner.displayName }));
          ...
          <WordSaveModal
            ...
            onSaved={(item, duplicated) =>
              showToast(
                duplicated
                  ? t("chat.word_save_exists", { term: item.word.term })
