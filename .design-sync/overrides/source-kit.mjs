...
// component from src/ â€” JSDoc and dir-derived group. Every enrichment miss
...
// Dir names that don't usefully group components â€” skip so the emitted path
// is `components/<group>/<Name>` not `components/components/<Name>`.
// forked from design-sync lib/source-kit.mjs — repo app-shape: 'features' la thu muc chua chung, khong phai ten nhom; de frontmatter category quyet dinh nhom
...
// No .d.ts â†’ scan src files for PascalCase value exports via ts-morph.
function deriveComponentsFromSrc(srcFiles) {
  ...
  for (const p of srcFiles) {
    ...
    for (const [name, decls] of sf.getExportedDeclarations()) {
      // `export default function Button()` is keyed as 'default' â€” recover
...
export async function resolvePackage(ctx) {
  ...
  // â”€â”€ 1. src/ discovery (best-effort; feeds enrichment + synth-entry fallback).
  ...
  // â”€â”€ 2. entry: dist if it exists, else synthesize from src/ (last resort).
  let entry = resolveDistEntry({ pkgDir: PKG_DIR, pkgJson, override: ENTRY_OVERRIDE, pkgName: PKG, soft: true });
  let synthEntry = false;
  if (!entry) {
    if (!srcRoot) {
      console.error(`[NO_DIST] ${PKG} has no built entry and no src/ to synthesize from â€” run its build.`);
      process.exit(1);
    }
    ...
    synthEntry = true;
    console.error(
      `[NO_DIST] no built entry â€” synthesizing from ${comps.length} src files (run the package's build for best results)`,
    );
  }

  // â”€â”€ 3. component list: from shipped .d.ts (authoritative when dist exists).
  ...
  for (const [k, v] of Object.entries(srcMap)) {
    ...
    // Names reach `<script>` blocks in the emitted HTML â€” reject anything
  ...
  if (!components.length) {
    if (cfg.cssEntry || existsSync(join(PKG_DIR, 'styles.css'))) {
      console.error('[ZERO_MATCH] no component exports â€” treating as tokens-only DS');
      return { shape: 'package', entry, components: [], tokensOnly: true };
    }
    console.error(`[ZERO_MATCH] no PascalCase exports in ${PKG} and no styles â€” nothing to sync`);
    process.exit(1);
  }

  // â”€â”€ 4. src/ enrichment per component. Every miss degrades to plain-dist.
  if (srcRoot) {
    for (const c of components) {
      // Pinned via config â†’ skip fuzzy-find entirely.
      ...
      // a generic container name â€” else JSDoc @category â€” else 'general'.
      c.group = slug(
  ...
  console.error(
    `  package: ${components.length} components` +
      (srcRoot ? ` (${components.filter((c) => c.srcPath).length} src-matched)` : ' (no src/ â€” dist-only)'),
  );
