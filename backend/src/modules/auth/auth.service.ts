...
export class AuthService {
  ...
  async register(
    ...
    let computedDisplayName = dto.displayName;
    if (!computedDisplayName) {
      const parts = [dto.lastName, dto.firstName].filter(Boolean);
      computedDisplayName = parts.length > 0 ? parts.join(' ') : dto.email.split('@')[0];
    }
